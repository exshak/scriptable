...
const weatherCal = {
  ...
  initialize(name, iCloudInUse) {
    ...
    this.widgetUrl = "https://raw.githubusercontent.com/exshak/scriptable/main/weather-cal.js"
    this.now = new Date()
    this.data = {}
  ...
  async runSetup(name, iCloudInUse, codeFilename, gitHubUrl) {
    ...
    if (!this.fm.fileExists(this.fm.joinPath(this.fm.libraryDirectory(), "weather-cal-setup")))
      return await this.initialSetup(backgroundSettingExists)
    if (backgroundSettingExists) return await this.editSettings(codeFilename, gitHubUrl)
    await this.generateAlert("Weather Cal is set up, but you need to choose a background for this widget.", [
      "Continue",
    ])
  ...
  async initialSetup(imported = false) {
    ...
    message =
      (imported ? "Welcome to Weather Cal. We" : "Next, we") +
      " need to check if you've given permissions to the Scriptable app. This might take a few seconds."
    await this.generateAlert(message, ["Check permissions"])

    let errors = []
    if (!(await this.setupLocation())) {
    ...
    if (errors.length == 3) {
      issues += ", " + errors[1] + ", and " + errors[2]
    }

    if (issues) {
      message =
        "Scriptable does not have permission for " +
        issues +
        ". Some features may not work without enabling them in the Settings app."
    ...
    if (weather == 1) {
      message =
        "On the next screen, sign up for OpenWeather. Find the API key, copy it, and close the web view. You will then be prompted to paste in the key."
    ...
    if (weather < 2 && !(await this.getWeatherKey(true))) {
      return
    }
  ...
  async editSettings(codeFilename, gitHubUrl) {
    ...
    if (response == menu.update) {
      if (
        await this.generateAlert("Would you like to update the Weather Cal code? Your widgets will not be affected.", [
          "Update",
          "Exit",
        ])
      )
      ...
      return await this.generateAlert(
        success ? "The update is now complete." : "The update failed. Please try again later.",
      )
    }
    ...
    if (response == menu.share) {
      ...
      const shouldUseQuickLook = await this.generateAlert("Your export is ready.", [
        "Save to Files",
        "Display as text to copy",
      ])
  ...
  async getWeatherKey(firstRun = false) {
    ...
    if (!apiKey || apiKey == "" || apiKey == null) {
      return await this.generateAlert("No API key was entered. Try copying the key again and re-running this script.", [
        "Exit",
      ])
    }
    ...
    if (apiResponse && apiResponse.current) {
      ...
      await this.generateAlert(
        "New OpenWeather API keys may take a few hours to activate. Your widget will start displaying weather information once it's active.",
        ["Continue"],
      )
    } else {
      return await this.generateAlert(
        "The key you entered, " + apiKey + ", didn't work. If it's a new key, it may take a few hours to activate.",
      )
    }
    return true
  },
  ...
  async getWeatherApiPath(newApiKey) {
    const apiParameter =
      "?appid=" +
      (newApiKey ||
        this.fm.readString(this.fm.joinPath(this.fm.libraryDirectory(), "weather-cal-api-key")).replace(/\"/g, ""))
  ...
  async setWidgetBackground() {
    ...
    if (backgroundType == 0) {
      ...
      const returnVal = await this.promptForText(
        "Gradient Colors",
        [background.initialColor, background.finalColor, background.initialDark, background.finalDark],
      ...
      if (!this.fm.fileExists(directoryPath) || !this.fm.isDirectory(directoryPath)) {
      ...
      background.dark = !(await this.generateAlert("Would you like to use a different image in dark mode?", [
        "Yes",
        "No",
      ]))
      if (background.dark)
        this.fm.writeImage(this.fm.joinPath(directoryPath, this.name + " (Dark).jpg"), await Photos.fromLibrary())
    }
  ...
  async createWidget(layout, name, iCloudInUse, custom) {
    ...
    if (!this.locale || this.locale == "" || this.locale == null) {
    ...
    if (custom && custom.background) {
      ...
      const extension =
        (this.darkMode && background.dark && !this.settings.widget.instantDark ? " (Dark)" : "") + ".jpg"
      const imagePath = this.fm.joinPath(
        this.fm.joinPath(this.fm.documentsDirectory(), "Weather Cal"),
        name + extension,
      )

      if (this.fm.fileExists(imagePath)) {
        if (this.fm.isFileStoredIniCloud(imagePath)) {
  ...
  async executeItem(item) {
    ...
    if (this[functionName]) {
      return await this[functionName](this.currentColumn, parameter)
    }
    console.error(
      "The " +
        functionName +
        " item in your layout is unavailable. Check for misspellings or other formatting issues. If you have any custom items, ensure they are set up correctly.",
    )
  },
    ...
    for (var i = 1; i < items.length - 1; i++) {
      ...
      if (!(this[trimmedItem] || (this.custom && this.custom[trimmedItem]))) {
        if (rawItem.match(/\s+\d+\s+/)) {
          ...
          if (value) {
            this.currentColumns[i].width = value
          }
          continue
        }
  ...
  right() {
    this.setAlignment(false, true)
  },
  left() {
    this.setAlignment(true, false)
  },
  center() {
    this.setAlignment(true, true)
  },
      ...
      .filter((event, index, array) => {
        if (
          !(
            index ==
            array.findIndex(
              (t) => t.identifier == event.identifier && t.startDate.getTime() == event.startDate.getTime(),
            )
          )
        ) {
          return false
        ...
        if (diff > 0 && this.now.getHours() < showFutureAt) {
          return false
        }
        ...
        if (
          calendars.length &&
          !(
            calendars.some((a) => a.identifier == event.calendar.identifier) || calendars.includes(event.calendar.title)
          )
        ) {
          return false
        }
        ...
        if (event.isAllDay) {
          return eventSettings.showAllDay
        }

        // If they leave it blank, set minutes after to the duration of the event
        const minutesAfter =
          parseInt(eventSettings.minutesAfter) >= 0
            ? parseInt(eventSettings.minutesAfter) * 60000
  ...
  async setupReminders() {
    ...
    this.data.reminders = reminders
      .filter((reminder) => {
        if (
          lists.length &&
          !(lists.some((a) => a.identifier == reminder.calendar.identifier) || lists.includes(reminder.calendar.title))
        ) {
          return false
        }
        if (!reminder.dueDate) {
          return reminderSettings.showWithoutDueDate
        }
        if (reminder.isOverdue) {
          return reminderSettings.showOverdue
        }
        if (reminderSettings.todayOnly) {
          return this.dateDiff(reminder.dueDate, this.now) == 0
        }
        return true
      })
  ...
  async setupLocation() {
    ...
    const locationCache = this.getCache(
      locationPath,
      this.settings ? parseInt(this.settings.widget.updateLocation) : null,
    )
    let location
    ...
    if (!locationCache || locationCache.cacheExpired) {
      try {
        ...
        location = locationCache || { cacheExpired: true }
      }
    ...
    if (!sunData || sunData.cacheExpired) {
      try {
        ...
        const sunReq =
          ...
          "&exclude=minutely,alerts&units=" +
          this.settings.widget.units +
          "&lang=" +
          locale
  ...
  async setupWeather() {
    ...
    if (!weatherData || weatherData.cacheExpired) {
      try {
        ...
        if (weatherData) {
          this.fm.writeString(weatherPath, JSON.stringify(weatherData))
        }
      } catch {}
    }
    ...
    this.data.weather.currentDescription = weatherData
      ? english
    ...
    for (let i = 0; i <= 7; i++) {
      this.data.weather.forecast[i] = weatherData
        ? {
            High: weatherData.daily[i].temp.max,
            Low: weatherData.daily[i].temp.min,
            Condition: weatherData.daily[i].weather[0].id,
          }
        : { High: null, Low: null, Condition: 100 }
      this.data.weather.hourly[i] = weatherData
        ? { Temp: weatherData.hourly[i].temp, Condition: weatherData.hourly[i].weather[0].id }
  ...
  async setupCovid() {
    ...
    if (!covidData || covidData.cacheExpired) {
      try {
        covidData = await new Request(
          "https://coronavirus-19-api.herokuapp.com/countries/" +
            encodeURIComponent(this.settings.covid.country.trim()),
        ).loadJSON()
  ...
  async date(column) {
    ...
    if (!this.data.events && dateSettings.dynamicDateSize) {
    ...
    if (settingUrl.trim() != "none") {
      dateSettings.url = settingUrl.length > 0 ? settingUrl : defaultUrl
    }

    if (dateSettings.dynamicDateSize ? this.data.events.length : dateSettings.staticDateSize == "small") {
      this.provideText(
        this.formatDate(this.now, dateSettings.smallDateFormat),
        column,
        this.format.smallDate,
        true,
        dateSettings.url,
      )
    } else {
      const dateOneStack = this.align(column)
      const dateOne = this.provideText(
        this.formatDate(this.now, dateSettings.largeDateLineOne),
        dateOneStack,
        this.format.largeDate1,
        false,
        dateSettings.url,
      )
      ...
      const dateTwo = this.provideText(
        this.formatDate(this.now, dateSettings.largeDateLineTwo),
        dateTwoStack,
        this.format.largeDate2,
        false,
        dateSettings.url,
      )
  ...
  greeting(column) {
    ...
    function makeGreeting(hour, localization) {
      ...
      if (hour < 12) {
        return localization.morningGreeting
      }
      if (hour - 12 < 5) {
        return localization.afternoonGreeting
      }
      if (hour - 12 < 10) {
        return localization.eveningGreeting
      }
      return localization.nightGreeting
    }
  ...
  async events(column) {
    ...
    if (this.data.events.length == 0) {
      const secondsForToday = Math.floor(new Date().getTime() / 1000) - 978307200
      if (eventSettings.noEventBehavior == "message" && this.localization.noEventMessage.length) {
        return this.provideText(
          this.localization.noEventMessage,
          column,
          this.format.noEvents,
          true,
          settingUrlExists ? eventSettings.url : "calshow:" + secondsForToday,
        )
      }
      if (this[eventSettings.noEventBehavior]) {
        return await this[eventSettings.noEventBehavior](column)
      }
    }
    ...
    for (let i = 0; i < numberOfEvents; i++) {
      ...
      if (diff != currentDiff) {
        ...
        const eventLabelText =
          diff == 1 && tomorrowText.length ? tomorrowText : this.formatDate(event.startDate, eventSettings.labelFormat)
  ...
  async reminders(column) {
    ...
    if (this.data.reminders.length == 0) {
      if (reminderSettings.noRemindersBehavior == "message" && this.localization.noRemindersMessage.length) {
        return this.provideText(this.localization.noRemindersMessage, column, this.format.noReminders, true)
      }
    ...
    for (let i = 0; i < numberOfReminders; i++) {
      ...
      if (reminder.isOverdue) {
        title.textColor = new Color(reminderSettings.overdueColor || "ff3b30")
      }
      if (reminder.isOverdue || !reminder.dueDate) {
        continue
      }
      ...
      if (reminderSettings.useRelativeDueDate) {
        ...
        if (this.dateDiff(reminder.dueDate, this.now) == 0 && reminder.dueDateIncludesTime) {
          df.useNoDateStyle()
        } else {
  ...
  async current(column) {
    ...
    if (weatherSettings.showLocation) {
      this.provideText(locationData.locality, currentWeatherStack, this.format.smallTemp, true)
    }

    const mainConditionStack = this.align(currentWeatherStack)
    const mainCondition = mainConditionStack.addImage(
      this.provideConditionSymbol(weatherData.currentCondition, this.isNight(this.now)),
    )
  ...
  async future(column) {
    ...
    if (showNextHour) {
      nightCondition = this.isNight(new Date(this.now.getTime() + 60 * 60 * 1000))
    }

    const subCondition = subConditionStack.addImage(
      this.provideConditionSymbol(
        showNextHour ? weatherData.hourly[1].Condition : weatherData.forecast[1].Condition,
        nightCondition,
      ),
    )
    ...
    if (showNextHour) {
      this.provideText(
        this.displayNumber(weatherData.hourly[1].Temp, "--") + "°",
        subConditionStack,
        this.format.smallTemp,
      )
    } else {
      const tomorrowLine = subConditionStack.addImage(
        this.drawVerticalLine(this.provideColor(this.format.tinyTemp, 0.5), 20),
      )
    ...
    if (weatherSettings.showRain) {
      ...
      this.provideText(
        this.displayNumber(showNextHour ? weatherData.nextHourRain : weatherData.tomorrowRain, "--") + "%",
        subRainStack,
        this.format.smallTemp,
      )
    }
  ...
  async forecast(column, hourly = false) {
    ...
    const [locationData, weatherData, sunData, weatherSettings] = [
      this.data.location,
      this.data.weather,
      this.data.sun,
    ...
    const smallFontSize =
      this.format.smallTemp && this.format.smallTemp.size ? this.format.smallTemp.size : this.format.defaultText.size
    ...
    for (var i = startIndex; i < endIndex; i++) {
      ...
      if (horizontal) {
        conditionStack.addSpacer()
      }

      // Set up the container for the condition.
      if (hourly) {
        const subCondition = conditionStack.addImage(
          this.provideConditionSymbol(weatherData.hourly[i].Condition, this.isNight(myDate)),
        )
        ...
        const temp = this.provideText(
          this.displayNumber(weatherData.hourly[i].Temp, "--") + "°",
          tempStack,
          this.format.smallTemp,
        )
        temp.lineLimit = 1
        ...
        const tinyFontSize =
          this.format.tinyTemp && this.format.tinyTemp.size ? this.format.tinyTemp.size : this.format.defaultText.size
        conditionStack.size = new Size(0, tinyFontSize * 2.64)

        const conditionIcon = conditionStack.addImage(
          this.provideConditionSymbol(weatherData.forecast[i - 1].Condition, false),
        )
        conditionIcon.imageSize = new Size(18, 18)
        this.tintIcon(conditionIcon, this.format.smallTemp)
        conditionStack.addSpacer(5)

        const tempLine = conditionStack.addImage(
          this.drawVerticalLine(this.provideColor(this.format.tinyTemp, 0.5), 20),
        )
        ...
        const tempHigh = this.provideText(
          this.displayNumber(weatherData.forecast[i - 1].High, "-"),
          tempStack,
          this.format.tinyTemp,
        )
        tempHigh.lineLimit = 1
        tempHigh.minimumScaleFactor = 0.6
        tempStack.addSpacer(4)
        const tempLow = this.provideText(
          this.displayNumber(weatherData.forecast[i - 1].Low, "-"),
          tempStack,
          this.format.tinyTemp,
        )
        tempLow.lineLimit = 1
  ...
  async sunrise(column, forceSunset = false) {
    ...
    const [sunrise, sunset, tomorrow, current, sunSettings] = [
      this.data.sun.sunrise,
      this.data.sun.sunset,
      this.data.sun.tomorrow,
      this.now.getTime(),
    ...
    if (
      showWithin > 0 &&
      !(Math.abs(this.now.getTime() - sunrise) / 60000 <= showWithin) &&
      !(Math.abs(this.now.getTime() - sunset) / 60000 <= showWithin)
    ) {
      return
    }
    ...
    const time = this.provideText(
      timeToShow == null ? "--" : this.formatTime(new Date(timeToShow)),
      sunriseStack,
      this.format.sunrise,
    )
  },
  ...
  async covid(column) {
    ...
    this.provideText(
      this.localization.covid.replace(/{(.*?)}/g, (match, $1) => {
  ...
  symbol(column, name) {
    ...
    if (symSettings.tintColor.length > 0) {
      symbol.tintColor = new Color(symSettings.tintColor)
    }
  },
  ...
  tintIcon(icon, format, force = false) {
    ...
    if (!force && (never || notDark || notLight)) {
      return
    }
  ...
  formatTime(date) {
    return this.formatDate(date, null, false, true)
  },
  formatDatetime(date) {
    return this.formatDate(date, null, true, true)
  },
  ...
  provideColor(format, alpha) {
    ...
    if (this.settings.widget.instantDark)
      return Color.dynamic(new Color(lightColor, alpha), new Color(darkColor, alpha))
  ...
  async defaultSettings() {
    const settings = {
      widget: {
        ...
        padding: {
          ...
          name: "Item padding",
          description:
            "The padding around each item. This also determines the approximate widget padding. Default is 5.",
        },
        widgetPadding: {
          ...
          type: "multival",
          description:
            "The padding around the entire widget. By default, these values are blank and Weather Cal uses the item padding to determine these values. Transparent widgets often look best with these values at 0.",
        },
        tintIcons: {
        ...
        updateLocation: {
          ...
          description:
            "How often, in minutes, to update the current location. Set to 0 to constantly update, or -1 to never update.",
        },
        instantDark: {
          val: false,
          ...
          type: "bool",
          description:
            "Instantly switch to dark mode. \u26A0\uFE0F This DOES NOT support dark mode image backgrounds or custom icon tint settings. \u26A0\uFE0F",
        },
      },
      localization: {
        ...
        covid: {
          ...
          description:
            "Each {token} is replaced with the number from the data. The available tokens are: cases, todayCases, deaths, todayDeaths, recovered, active, critical, casesPerOneMillion, deathsPerOneMillion, totalTests, testsPerOneMillion",
        },
        week: {
          val: "Week",
      ...
      date: {
        ...
        url: {
          ...
          description:
            "Optionally provide a URL to open when this item is tapped. Leave blank to open the built-in Calendar app.",
        },
      },
      events: {
        name: "Events",
        ...
        minutesAfter: {
          ...
          description:
            "Number of minutes after an event begins that it should still be shown. Leave blank for an event to show for its duration.",
        },
        showAllDay: {
          val: false,
        ...
        showTomorrow: {
          ...
          description:
            "The hour (in 24-hour time) to start showing events for tomorrow or beyond. Use 0 for always, 24 for never.",
        },
        showEventLength: {
          val: "duration",
      ...
      reminders: {
        ...
        url: {
          ...
          name: "URL to open when tapped",
          description:
            "Optionally provide a URL to open when this item is tapped. Leave blank to open the built-in Reminders app.",
        },
      },
      sunrise: {
        name: "Sunrise and sunset",
        showWithin: {
          ...
          description:
            "Set how many minutes before/after sunrise or sunset to show this element. Leave blank to always show.",
        },
        separateElements: {
          val: false,
          name: "Use separate sunrise and sunset elements",
          description:
            "By default, the sunrise element changes between sunrise and sunset times automatically. Set to true for individual, hard-coded sunrise and sunset elements.",
          type: "bool",
        },
      },
      weather: {
        ...
        locale: {
          ...
          description:
            "If you are encountering issues with your weather data, try choosing an OpenWeather locale code.",
          type: "enum",
        ...
        tomorrowShownAtHour: {
          ...
          description:
            "Set the hour (in 24-hour time) to switch from the next hour to tomorrow's weather. Use 0 for always, 24 for never.",
        },
        spacing: {
          val: "0",
    ...
    async function getFromCalendar(forReminders) {
      try {
        return (await forReminders) ? Calendar.forReminders() : Calendar.forEvents()
      } catch {
        return []
      }
    }
...
if (moduleName == Script.name()) {
  await (async () => {
    ...
    await weatherCal.runSetup(
      name,
      true,
      "Weather Cal code",
      "https://raw.githubusercontent.com/exshak/scriptable/main/weather-cal-code.js",
    )
